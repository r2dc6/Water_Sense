...
#         print(f"Shift Register Value: x= {value_0[:4]} y= {value_0[4:]} z= {value_1[:4]} a= {value_1[4:]}")
    ...
    def move(self, x=0, y=0, a=0, g=0):
        ...
        for i in range(max(x, y, z, a, g)):
            ...
            self.stepper_g_value = self.stepper_g.getPos() if g == 0 or g <= i else self.stepper_g.move(gDir)
            
            self._shift_out(value_0=self.stepper_y_value+self.stepper_a_value, value_1=self.stepper_x_value+self.stepper_z_value,value_2=self.stepper_g_value+self.stepper_g_value)
            sleep(0.001)

    def cleanup(self):
        GPIO.cleanup()
# 
cnc = ShiftReg(data_0=24, data_1=25, data_2=8, clock=4, latch=23)
cnc.move(x=0, y=0, a=0, g=2000)
print("done")
cnc.cleanup()
